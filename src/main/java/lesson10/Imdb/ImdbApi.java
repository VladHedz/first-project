...
public class ImdbApi {
    ...
    public LinkedHashMap<String, String> getTopChart() throws IOException {
        var htmlString = getTopChartResponse().string();
        var document = Jsoup.parse(htmlString);
        var ipcTitleElements = document.select(".cli-children");
        //var cliTitleElements = document.select(".cli-title-metadata");
        ...
        /*for (Element cliTitleElement : cliTitleElements) {
            var aTag = cliTitleElement.select("div").first();
            if (aTag != null) {
                var year = aTag.selectFirst("span").text();
                var href = aTag.attr("href");
                var link = ImdbRestClient.BASE_URL + href;
                allFilms.put(year, link);
            }
        }*/
        return allFilms;
    }
